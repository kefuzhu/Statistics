...
date: "Wednesday, September 17, 2016"
output:
  html_document:
    highlight: tango
    toc: yes
...
Do Not remove any of the comments. These are marked by # The .R file will contain your code and answers to questions.
...
Examine the following code, and briefly explain what it is doing.
...
**Answer**: For each element in list y, myfunc turns it into three three elements: the original element, the square of it, and the integer division value of the cubic value divided by 2.
...
**Answer**: It transform the list-presented value from myfunc(y) to a matrix with 3 colummns, where each row represents the outputs from an original element in y

## (2) 

Simplify the code in (1) using one of the "apply" functions and save the result as m.
...
Find the column sum of m in two ways.
...
Could you divide all the values by 2 in two ways?
...
What is the product of the values in each element?
...
What is the (sample) variance of the values in each element?
...
First change l and l.2 into matrixes, make each element in the list as column,
...
Find the mean bmi by gender in one function call.
...
Could you get a vector of what the type of variables the dataset is made of?
...
Could you sort the firstName in height descending order?
...
Now obtain the sum of the first 4 variables, by species, but using only one function call.
...
**Answer**: Based on the previous diagnostic plots, no assumptions are obviously violated. The linear model is valid. Based on the summary of fitted linear model, the weight of a person is expected to increase by 9.154 units if the height of that person is increased by 1 unit.
...
**Answer**: The plot indicates a positive linear relationship between fheight and fweight. And the line in the plot is the best fitted linear regression model with least sum of square of residuals.
