...
boot.sample <- function(sample.size, B) {
  ...
  colnames(confidence_95) <-
    c('Lower confidence interval',
  ...
  if (count < dim(confidence_95)[1]) {
    for (i in 1:length(sample.size)) {
      for (j in 1:length(B)) {
        ...
        set.seed(0)
        lambda_bootstrap <- replicate(B[j], 1 / mean(rexp(n = sample.size[i], lambda_hat)))
        # Estimate of standard error of lambda
        se_bootstrap <- sd(lambda_bootstrap)
        # 95% confidence interval for bootstrap estimate of lambda
        confidence_95_tmp <- c(
...
# Answer: John Tukey does predict the incoming of data science in some ways. When John discuss
#         about how would the new data analysis be initiated, he mentions that people will 
#         very likely to have concerns on "more complexly organized data". In traditional
#         statistics, the data is often generalized from experiments or surveys on random sampling,
#         where statisticians can control how organized the data will be by desgining the experiment and the survey.
#         However, John brings out the idea of "more complexly organized data" at that time, which indeed corresponds to
#         one feature of Big Data nowadays: The raw data from the Internet is messy and unorganized.
#
#         John also mentions the change of focus in data analysis in his article: "Procedures of diagnosis, 
#         and procedures to extract indications rather than conclusions, 
#         will have to playa large part in the future of data analyses". In fact, today's data scientist
#         usually spend 80-90% of their time on data mining and preparing the data in certain form for
#         any further data analysis. John also predicts the automation of preliminary analysis, where he says
#         graphs (data visualization nowadays) "will cease to be the vehicle through which 
#         a man diagnoses or seeks indications, becoming, instead, the vehicle through 
#         which the man supervises, and approves or disapproves, the diagnoses and indications already found by the machine"
#         Newly rising machine learning techniques basically do the same thing as John has described it in his article.
#
#         In addition, John also emphasizes the impact of developing computing power, he claims that
#         the computer, in some situations, can makes unfeasible things feasible. And even for feasible things,
#         computer can also be vital in terms of speed and economy of finding the answer.
...
# Answer: The way John describes his central interest is very similar to the way the Life Cycle of Data
#         is defined now. "All in all, I have come to feel that my central interest is 
#         in data analysis, which I take to include, among other things: procedures for 
#         analyzing data, techniques for interpreting the results of such procedures, 
#         ways of planning the gathering of data to make its analysis easier, more precise 
#         or more accurate, and all the machinery and results of (mathematical) statistics 
#         which apply to analyzing data." 

#         This description includes several parts of Life Cycle of Data:
#         data aquisition, data cleaning and data organization, (ways of planning the gathering of data to make its analysis easier, more precise, or more accurate); 
#         data analysis (all the machinery and results of (mathematical) statistics which apply to analyzing data);
#         and data reporting (techniques for interpreting the results of such procedures).
#